...
import { EXPECTED_GET_VERSION_RESPONSE, EXPECTED_PK, EXPECTED_SIGN_MESSAGE_RESPONSE, GET_ADDRESS_RESPONSE_APDU, GET_VERSION_RESPONSE_APDU, MSG_DATA, SIGN_MESSAGE_RESPONSE_APDU, SIGN_TRANSACTION_RESPONSE_APDU, TX_DATA } from '../helper';
...
export const runLedgerTests = (environment: string) => {
    describe(`Ledger ${environment} Integration Tests`, () => {
        ...
        it("Sign message", async () => {
            ...
            const transport = new MockTransport(responseBuffer);
            const app = new MinaApp(transport);
            const resp = await app.signMessage(MSG_DATA.msg.account, MSG_DATA.msg.networkId, MSG_DATA.msg.msg);

            expect(resp.field).toEqual(EXPECTED_SIGN_MESSAGE_RESPONSE.signature.field);
            expect(resp.scalar).toEqual(EXPECTED_SIGN_MESSAGE_RESPONSE.signature.scalar);
        });
    });
}; 
